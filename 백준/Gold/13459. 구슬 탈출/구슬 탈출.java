...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        setVariables();
        dfs(0, red.r, red.c, blue.r, blue.c);
        System.out.println(clear ? 1 : 0);
    }

    private static void dfs(int depth, int redRow, int redCol, int blueRow, int blueCol) {
        if(clear) return;
        if (depth == 10) {
            return;
        }
        ...
        if (!move.blueFinish) {
            if (!move.redFinish) {
                ...
                if (!(redRow == nextRed.r && redCol == nextRed.c && blueRow == nextBlue.r
                        && blueCol == nextBlue.c)) {
                    dfs(depth + 1, nextRed.r, nextRed.c, nextBlue.r, nextBlue.c);
                }
            } else {
                clear  = true;
                return;
